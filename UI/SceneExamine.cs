...
public class SceneExamine: IScene {
	...
	Vec3D modelPosition = new Vec3D(0, 0, 2);
    ...
    public void Initialize() {
		...
		consoleWidth = Screen.WindowSize.X;
		consoleHeight = Screen.WindowSize.Y;
    ...
    public void Update() {
        if (Input.GetKeyDown(ConsoleKey.Y)) {
	    	drawWireframe = !drawWireframe;
	    }
	    if (Input.GetKeyDown(ConsoleKey.H)) {
	    	drawSolid = !drawSolid;
	    }

	    if(Input.GetMouseLeft()) {
	    	Point delta = Input.GetMousePos() - lastMousePos;
	    	yRot += delta.X / 80f;
	    	xRot += delta.Y / 80f;
	    }
	    ...
	    for (int i = 0; i < mesh.Triangles.Length; i++) {
	    	...
	    	if (Vec3D.Dot(normal, transformed.p[0] - camera) < 0.0f) {
	    		...
	    		char character = (char)0x2591; //light
	    		if (l > 0.4) character = (char)0x2592; //medium
	    		if (l > 0.7) character = (char)0x2593; //dark
	    		if (l > 1) character = (char)0x2588; //full
    ...
    public void Render() {
        foreach(Triangle t in trianglesToRaster) {
			...
			if(drawSolid) Screen.FillTriangle(b, a, c, 9, t.c);
			if(drawWireframe) Screen.Triangle(b, a, c, 7, t.c);
		}
